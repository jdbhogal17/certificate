...
function animateteksnim() {
    let percent = 0;
    const step = 20; // increase by 20 each time (0,20,40,60,80,100)

    const intervalId = setInterval(() => {
        if (percent <= 100) {
            const randomEmoji = emojiAkhir[Math.floor(Math.random() * emojiAkhir.length)];
            teksLove.innerHTML = `<b>${teksLoveBawaan} ${randomEmoji}</b>`;
    ...
    }, 500); // speed (0.5s between updates)
}
...
function katanimasi3() {
    ...
    const stringsArray = steps.map((percent, i) => {
        const emoji = shuffledEmojis[i % shuffledEmojis.length];
        return `I will keep you happy ${percent}% ${emoji}`;
    });
    ...
    new TypeIt("#teks4", {
        ...
        afterComplete: function () {
            ...
            teksLove.innerHTML += ' 100% ' + randomEmot;
